...
CONFIG = {
    ...
    "API_HEADERS": {
        "Origin": "https://rakuten.ne.jp",
        "Referer": "https://rakuten.ne.jp",
...
def search_items(app_id, access_key, api_base, watch):
    ...
    try:
        resp = requests.get(api_base, params=params, headers=CONFIG["API_HEADERS"], timeout=15)
        resp.raise_for_status()
        items_raw = resp.json().get("Items", [])
    except requests.exceptions.HTTPError as e:
        status_code = e.response.status_code if e.response is not None else 0
        if status_code == 429:
            print(f"[WARN] API レート制限 (429)", flush=True)
        elif status_code == 404:
            print(f"[WARN] API 対象なし (404)", flush=True)
        elif status_code in (500, 503):
            print(f"[WARN] API サーバーエラー ({status_code})", flush=True)
        else:
            print(f"[WARN] API HTTP エラー ({status_code}): {e}", flush=True)
        return []
...
def matches(item, watch):
    ...
    if "中古" not in item["name"] and "中古" not in item["caption"]:
        return False
...
def _run(supabase, now_jst):
    app_id = os.environ["RKT_APP_ID"]
    access_key = os.environ["RKT_ACCESS_KEY"]
    api_base = os.environ["RKT_API_BASE"]
